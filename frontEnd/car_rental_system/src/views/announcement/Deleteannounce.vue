<template>
        ...
        <el-table :data="tableData" :height="tableHeight" border style="width: 100%">
          ...
          <el-table-column>
            <template slot-scope="scope">
              <el-button type="primary" size="small" @click="ANEdit(scope.row.id)">发布</el-button>
              <el-button type="danger" size="small" @click="ANDelete(scope.row.id, scope.row.type)">删除</el-button>
          </template>
    ...
    // 这个操作都应该放进Vue.nextTick()的回调函数中
     ...
     ANEdit(id){
      axios.post("http://localhost:8081/announcement/release", {"id": id}).then(res =>{
        console.log(res.data)
        axios.post("http://localhost:8081/announcement/GetMyAnnouncement", {"user_id": this.user.user_id}).then(res =>{
          console.log(res.data)
          this.tableData = res.data
        })
      })
     ...
     ANDelete(id, type){
        if(type == 0)
        {
          axios.post("http://localhost:8081/announcement/delete", {"id": id}).then(res =>{
        console.log(res.data)
        axios.post("http://localhost:8081/announcement/GetMyAnnouncement", {"user_id": this.user.user_id}).then(res =>{
          console.log(res.data)
          this.tableData = res.data
