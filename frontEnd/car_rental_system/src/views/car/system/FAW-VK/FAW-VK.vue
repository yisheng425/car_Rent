...
<!--表格
    ...
    prop：字段属性需要跟表格的数据对应
    -->
    <el-table :data="tableData.slice((currentpage1-1)*pagesize,currentpage1*pagesize)" :height="tableHeight" border style="width: 100%">
      ...
      <el-table-column prop="inventory" label="状态"></el-table-column>
</el-table>
     <!-- 分页组件-->
     <el-pagination
...
// 这个操作都应该放进Vue.nextTick()的回调函数中
  mounted() {
   ...
   methods: {
     ...
     search(){
       axios.get('http://localhost:3000/FAW-VK?carNo_like='+this.userForm.userName)
       .then((res)=>{
